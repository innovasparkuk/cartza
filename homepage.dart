...
//  ✅ CENTRALIZED layout — same pattern as CartPage (maxWidth: 1200)
//  ✅ Full-width backgrounds with centered content containers
//  ✅ AppBar top strip and main bar both centered at maxWidth: 1200
//  ✅ ALL sections use Center + ConstrainedBox(maxWidth: 1200)
//  ✅ DeliveryStrip, FilterChips, CatSection, FlashHeader, HList,
//     PromoBanners, ProdGrid, Footer — all centralized
//  ✅ Body uses CustomScrollView with SliverToBoxAdapter per section
//     so each section has full-width bg + centered content
//  ✅ Banner fully visible — ratio-based, never cut off
//  ✅ Responsive for all screen sizes
//  ✅ Flash Sale & Recommended open ProductDetailPage on tap
//  ✅ Search bar functional – filters and navigates
// ============================================================
import 'package:flutter/material.dart';
import 'package:cartza/category_page.dart';
import 'package:cartza/notifications_page.dart' hide notificationService;
import 'package:cartza/profile_page.dart';
import 'package:cartza/cart_page.dart';
...
class _BP {
  ...
  static const maxW = 1200.0; // ← same as CartPage
  ...
  // Horizontal padding inside the centered container
...
const _kFlash = <_Prod>[
  ...
  _Prod(name: 'Tattoo Needles', price: 30,  imageAsset: 'assets/images/subcategory_3_8.png', description: 'High-quality needles for professional tattoo application.',       category: 'Mehndi & Tattooing',  badge: '30% OFF', rating: 4.4, sold: 189),
  _Prod(name: 'Eye Liner',      price: 18,  imageAsset: 'assets/images/subcategory_1_9.png', description: 'Smudge-proof eyeliner for precise and long-lasting wear.',        category: 'Beauty & Cosmetics',  badge: 'HOT',     rating: 4.5, sold: 231),
  _Prod(name: 'Headphones',     price: 50,  imageAsset: 'assets/images/subcategory_5_3.png', description: 'Noise-cancelling and wireless headphones for premium sound.',     category: 'Electronics',          badge: '25% OFF', rating: 4.8, sold: 178),
  _Prod(name: 'Mehndi Cones',   price: 5,   imageAsset: 'assets/images/subcategory_3_1.png', description: 'Natural henna cones for intricate mehndi designs.',              category: 'Mehndi & Tattooing',  badge: 'NEW',     rating: 4.3, sold: 95),
];

const _kRec = <_Prod>[
  _Prod(name: 'Skin Care',        price: 25, imageAsset: 'assets/images/subcategory_1_1.png', description: 'Skin care products enriched with natural oils and vitamins.',   category: 'Beauty & Cosmetics',  badge: 'NEW', rating: 4.6, sold: 87),
  _Prod(name: 'Handmade Jewelry', price: 45, imageAsset: 'assets/images/subcategory_2_1.png', description: 'Unique handmade jewelry crafted with care and creativity.',     category: 'Handmade Products',   badge: '',    rating: 4.3, sold: 145),
  _Prod(name: 'Cricket Bat',      price: 35, imageAsset: 'assets/images/subcategory_8_7.png', description: 'Premium cricket bats for practice and matches.',                category: 'Sports',               badge: 'HOT', rating: 4.5, sold: 312),
  _Prod(name: 'Fiction Books',    price: 15, imageAsset: 'assets/images/subcategory_9_1.png', description: 'Engaging fiction books across various genres.',                 category: 'Books',                badge: 'SALE',rating: 4.4, sold: 204),
  _Prod(name: 'Clay Pots',        price: 15, imageAsset: 'assets/images/subcategory_2_2.png', description: 'Durable and stylish clay pots for home decoration.',           category: 'Handmade Products',   badge: '',    rating: 4.7, sold: 67),
  _Prod(name: 'Kids Clothing',    price: 25, imageAsset: 'assets/images/subcategory_4_2.png', description: 'Comfortable and stylish clothing for children of all ages.',   category: 'Kids',                 badge: 'NEW', rating: 4.8, sold: 198),
];
...
class _HomepageState extends State<Homepage> {
  ...
  void _goToCategory([String? name]) {
    Navigator.push(context, MaterialPageRoute(builder: (_) => CategoryPage(selectedCategory: name)));
  }
  ...
  void _onSearch(String q) {
    ...
    Navigator.push(context, MaterialPageRoute(
        builder: (_) => CategoryPage(selectedCategory: q.trim())));
  }

  void _showFlashSalePage() {
    Navigator.push(context, MaterialPageRoute(
      builder: (_) => _FlashSalePage(products: _kFlash, onProductTap: _goToProduct),
    ));
  }

  void _showRecommendPage() {
    Navigator.push(context, MaterialPageRoute(
      builder: (_) => _RecommendPage(products: _kRec, onProductTap: _goToProduct),
    ));
  }
...
// ══════════════════════════════════════════════════════════════
// DELIVERY STRIP
// ✅ Rendered inside a centered container — same as CartPage
// (background color is set by the _section helper in the body)
// ══════════════════════════════════════════════════════════════
class _DeliveryStrip extends StatelessWidget {
  static const _items = [
    [Icons.local_shipping_outlined, 'Free delivery Rs.2000+'],
...
// ══════════════════════════════════════════════════════════════
// MOBILE BANNER — ratio-based height, never cropped
// ══════════════════════════════════════════════════════════════
class _MobileBanner extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    final mob = _BP.isMob(context);
    return Padding(
      padding: EdgeInsets.symmetric(horizontal: mob ? 10 : 14, vertical: 10),
      child: ClipRRect(
        borderRadius: BorderRadius.circular(mob ? 10 : 14),
        child: LayoutBuilder(builder: (ctx, constraints) {
          final bannerH = (constraints.maxWidth / 2.4).clamp(140.0, 340.0);
          return SizedBox(
            width: double.infinity,
            height: bannerH,
            child: _BannerSlider(height: bannerH),
          ...
          return Stack(fit: StackFit.expand, children: [
            ...
            Positioned(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                  mainAxisSize: MainAxisSize.min, children: [
                    Text(slide['title']!, style: TextStyle(
                        color: Colors.white, fontSize: mob ? 18 : 24,
...
// ══════════════════════════════════════════════════════════════
// DESKTOP HERO ROW — sidebar + banner, fully visible, no crop
// Content already inside a centered ConstrainedBox from _section
// ══════════════════════════════════════════════════════════════
class _DesktopHeroRow extends StatelessWidget {
  final void Function([String?]) onCat;
  const _DesktopHeroRow({required this.onCat});
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: LayoutBuilder(builder: (ctx, constraints) {
        const sideW = 220.0;
        final availW = constraints.maxWidth - sideW - 12;
        final bannerH = (availW / 2.6).clamp(160.0, 320.0);
                  ...
                  child: Image.network(cat['img'] as String, fit: BoxFit.cover,
                      errorBuilder: (_, __, ___) => Container(
                          ...
                          child: Icon(cat['icon'] as IconData,
                              color: cat['color'] as Color, size: 14))),
                )),
                const SizedBox(width: 8),
                Expanded(child: Text(cat['name'] as String,
                    maxLines: 1, overflow: TextOverflow.ellipsis,
      ...
      InkWell(
        ...
        child: Container(width: double.infinity,
          ...
          decoration: BoxDecoration(color: _C.green.withOpacity(0.06),
              borderRadius: const BorderRadius.vertical(bottom: Radius.circular(10))),
          child: const Center(child: Text('See All Categories',
              style: TextStyle(color: _C.green, fontSize: 11.5,
...
// ══════════════════════════════════════════════════════════════
// CATEGORIES SECTION
// Content is already inside a centered ConstrainedBox from _section
// ══════════════════════════════════════════════════════════════
                ...
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                        shape: BoxShape.circle,
                        border: Border.all(
                            color: _C.green.withOpacity(0.3), width: 2),
                        boxShadow: [BoxShadow(
                            color: Colors.black.withOpacity(0.08),
                            blurRadius: 6, offset: const Offset(0, 2))]),
                    child: ClipOval(child: Image.network(cat['img'] as String,
                        fit: BoxFit.cover,
                        errorBuilder: (_, __, ___) => Container(
                            color: (cat['color'] as Color).withOpacity(0.12),
                            child: Icon(cat['icon'] as IconData,
                  ...
                  Flexible(child: Text(cat['name'] as String,
                    textAlign: TextAlign.center,
                    maxLines: 2, overflow: TextOverflow.ellipsis,
                    style: TextStyle(fontSize: mob ? 9.5 : 10,
                        fontWeight: FontWeight.w600,
                        color: _C.slate, height: 1.2),
                  )),
                ]),
              ),
            );
              ...
              child: Container(
                ...
                child: Column(children: [
                  ...
                  Padding(padding: const EdgeInsets.fromLTRB(6, 5, 6, 6),
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                        mainAxisSize: MainAxisSize.min, children: [
                          Text(cat['name'] as String, maxLines: 1,
      ...
      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Row(children: [
          ...
          Text(title, style: const TextStyle(
              fontSize: 15.5, fontWeight: FontWeight.w800, color: _C.slate)),
        ]),
        TextButton.icon(
          onPressed: onAction,
...
class _PromoBanners extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final hp   = _BP.hp(context);
    final desk = _BP.isDesk(context);
    return Padding(padding: EdgeInsets.fromLTRB(hp, 4, hp, 4),
      child: Row(children: [
        ...
        if (desk) ...[
          ...
          _PBanner(title: 'Sell on Cartza', sub: 'Start your business today',
              g1: _C.darkGreen, g2: _C.green,
              icon: Icons.storefront_outlined, onTap: onSell),
        ],
      ]),
    );
  }
...
class _PBanner extends StatelessWidget {
  ...
  @override
  Widget build(_) => Expanded(child: InkWell(onTap: onTap,
    borderRadius: BorderRadius.circular(12),
    child: Container(height: 82, padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(
          gradient: LinearGradient(colors: [g1, g2],
              begin: Alignment.topLeft, end: Alignment.bottomRight),
          borderRadius: BorderRadius.circular(12),
          boxShadow: [BoxShadow(color: g1.withOpacity(0.28),
              blurRadius: 10, offset: const Offset(0, 4))]),
      child: Row(children: [
                            ...
                            children: [
                              ...
                              Text(p.name, textAlign: TextAlign.center, maxLines: 2,
                                  style: TextStyle(fontSize: 9,
                                      color: _C.slate.withOpacity(0.6))),
                            ]),
                      ),
                    ),
                  ),
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(p.name, maxLines: 1, overflow: TextOverflow.ellipsis,
                          style: TextStyle(fontSize: mob ? 10.0 : 11.0,
                              fontWeight: FontWeight.w700,
                              color: _C.slate, height: 1.2)),
                      const SizedBox(height: 2),
                      Text('★ ${p.rating.toStringAsFixed(1)}  •  ${p.sold} sold',
                          maxLines: 1, overflow: TextOverflow.ellipsis,
                          style: const TextStyle(fontSize: 9.0,
                              color: _C.star, fontWeight: FontWeight.w600)),
                      const SizedBox(height: 3),
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          Text('Rs.${p.price.toStringAsFixed(0)}',
                              style: const TextStyle(color: _C.orange,
    ...
    return Scaffold(
      ...
      body: Column(children: [
        Container(
          ...
          child: const Row(children: [
            Icon(Icons.timer_outlined, color: _C.orange, size: 16),
            SizedBox(width: 6),
            Text('Limited time deals — shop before they expire!',
        ...
        Expanded(
          child: GridView.builder(
            ...
            itemBuilder: (_, i) => _PCard(
              prod: products[i], cw: double.infinity, gridMode: true,
              onAdd: () {}, onTap: () => onProductTap(products[i]),
            ),
          ),
        ),
      ]),
...
// ══════════════════════════════════════════════════════════════
// FOOTER
// ✅ Already inside a centered ConstrainedBox from the body _section
// ══════════════════════════════════════════════════════════════
        ...
        Padding(padding: const EdgeInsets.symmetric(vertical: 14),
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            const Flexible(child: Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(Icons.copyright, color: Colors.white38, size: 12),
              SizedBox(width: 4),
            ...
            Row(children: [
              const Text('We Accept: ',
                  style: TextStyle(color: Colors.white60, fontSize: 11)),
              ...[Icons.credit_card, Icons.account_balance_wallet, Icons.payment]
                  .map((ic) => Padding(padding: const EdgeInsets.only(left: 6),
...
class _FooterDesktop extends StatelessWidget {
  ...
  Widget build(_) => Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Expanded(flex: 2, child: Column(crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
              decoration: BoxDecoration(color: _C.green,
                  borderRadius: BorderRadius.circular(8)),
    ...
    Expanded(child: _FC('Shop',
        ['New Arrivals', 'Best Sellers', 'Flash Sale', 'Categories', 'Deals'])),
    Expanded(child: _FC('Customer Care',
        ['Contact Us', 'Help Center', 'Track Order', 'Returns', 'Shipping'])),
    Expanded(child: _FC('Company',
        ['About Us', 'Careers', 'Privacy Policy', 'Terms', 'Cookies'])),
...
class _FooterMobile extends StatelessWidget {
  ...
  Widget build(_) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    Wrap(spacing: 8, runSpacing: 8,
        children: ['Shop', 'Track', 'Contact', 'About', 'Privacy', 'Terms']
            .map((t) => InkWell(onTap: () {}, child: Container(
            padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
            decoration: BoxDecoration(color: Colors.white.withOpacity(0.05),
                borderRadius: BorderRadius.circular(6),
                border: Border.all(color: _C.green.withOpacity(0.25))),
            child: Text(t, style: const TextStyle(
                color: Colors.white60, fontSize: 11))))).toList()),
    const SizedBox(height: 16),
    _NL(),
  ]);
}
  ...
  Widget build(_) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    ...links.map((l) => Padding(padding: const EdgeInsets.only(bottom: 7),
        child: InkWell(onTap: () {}, child: Row(mainAxisSize: MainAxisSize.min, children: [
          Icon(Icons.arrow_right_rounded, size: 13,
...
class _NL extends StatelessWidget {
  ...
  Widget build(_) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    const Text('Newsletter', style: TextStyle(color: Colors.white,
        fontWeight: FontWeight.w700, fontSize: 12.5)),
    const SizedBox(height: 8),
    const Text('Get special offers & updates',
        style: TextStyle(color: Colors.white60, fontSize: 11.5)),
    const SizedBox(height: 10),
    Container(
      decoration: BoxDecoration(color: Colors.white.withOpacity(0.05),
